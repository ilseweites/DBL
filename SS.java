...
class SS {
    ...
    void count(int row, int column){
        // TODO
        //loop which runs for length of sudoku
        for(int i = 1; i <= SUDOKU_SIZE; i++ ){
            //if the current number is not in conflict with any other number in asterisk, box, row or column
            if(!givesConflict(row, column, i)){
                grid[row][column] = i; //changes the cell at index of row and column to given number
                //if the last cell in the sudoku is reached
                if(row == 8 && column == 8){
                    solutionCounter++; //adds another solution to the solution counter
                //else it looks for the next empty cell in the sudoku 
                } else {
                    
                    int[] coordinates = findEmptySquare(); //stores the coordinates of the empty cells (following the first cell) in an int array
                    int r = coordinates[0]; //stores the coordinates of the empty cells in an int array
                    int c = coordinates[1]; //stores column-coordinate of the empty cell in c
                    count(r, c); //calls back to count with the given coordinate of the empty cell
                }
            } 
        }
        grid[row][column] = 0; //resets the cell to zero if there is a conflict
    }
                ...
                if (grid[r][c]==0) {
                    ...
                    for (int d = SUDOKU_MIN_NUMBER; d <= SUDOKU_MAX_NUMBER; d++) {
                        ...
                        if (!givesConflict(r, c, d)) {
                            ...
                            //starts backtracking recursively
                            if(solve()){
                                return true; //if this is a solution, return true
                            } else {
                                grid[r][c]=0; //if this is not a solution, it empties the cell and continues
                            }
                        }
                    }
                    return false; //if the puzzle is unsolveable return false (there is no valid number to fill in for the empty cell)
                }
            }
        }
    ...
    void solveIt() {
        // TODO
        int[] coordinates = findEmptySquare(); //stores the coordinates of the first empty cell in an int array
        int r = coordinates[0]; //stores row-coordinate of the empty cell in r
        int c = coordinates[1]; //stores column-coordinate of the empty cell in c
        count(r, c); //calls back to count with the given coordinate of the empty cell

        //if there is more than one solution
        if (solutionCounter>1) {
        System.out.println(solutionCounter); //prints the amount of solutions
        }
        solve(); //calls back to solve() to solve the sudoku
        //if there is only one solution
        if (solutionCounter == 1) { 
        print(); //prints the single solution of the sudoku
        }
    }
